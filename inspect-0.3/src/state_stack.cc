...
size_t StateStack::prefixDepth() {
  size_t nonBacktrackCount;
  nonBacktrackCount = 0;
  assert(stack.size() != 0);
  for (auto it = stack.rbegin(); it != stack.rend(); ++it) {
    if ((*it)->backtrack.empty()) {
      nonBacktrackCount++;
    }
    else {
      // found a node with a backtrack point, this is where the prefix ends
      break;
    }
  }

#ifdef MK_DEBUG
  cerr << "nonBacktrackCount == " << nonBacktrackCount << '\n';
  cerr << "stack.size() == " << stack.size() << '\n';
#endif

  if (nonBacktrackCount == stack.size()) {
    assert(!has_backtrack_points());
  }

  assert(nonBacktrackCount <= stack.size());
  return stack.size() - nonBacktrackCount;
}
