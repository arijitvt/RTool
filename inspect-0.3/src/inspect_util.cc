...
{
#ifdef MK_DEBUG
  cout << "[MK DEBUG] Socket::send_string() sending string of size: " << str.size() << '\n';
#endif
...
#ifdef MK_DEBUG
  cout << "[MK DEBUG] Socket::recv_string() length = " << len << '\n';
#endif
  assert(retval != -1);
...
#ifdef MK_DEBUG
    cout << "[MK DEBUG] Socket::recv_string() bytes recieved from socket: " << retval << '\n';
#endif
...
#ifdef MK_DEBUG
    cout << "[MK DEBUG] recv_string() std::string assignment size == " << str.size() << '\n';
#endif
  }
