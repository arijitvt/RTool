...
/**
 * Consider the two vectors in this function to be the merger (the object this
 * member function is being invoked on) and the mergee (another_vec).
 *
 * This fuction will resize the merger to be the size of another_vec if
 * another_vec is larger than the merger.
 *
 * Then, for each item that is in another_vec the max of the item in
 * another_vec and the merger is stored into the merger (at this point, the
 * merger is guaranteed to be atleast as large as another_vec) 
 */
void ClockVector::merge_and_resize(ClockVector &another_vec) {
  resize(::max(width, another_vec.width));

  for (auto i = 0; i != another_vec.width; ++i) {
    assert(i < timestamps.size() && "out-of-bounds access");
...
#endif
  // invoke assignment operation on internal vectors of both lists
  cvs.vecs = vecs;
}
