...
#define ST_NUMHASH(x,size) (ABS((long)(((long)x)==INT_MIN ? ((long)0) : ((long)x)))%(size))
...
static int rehash ARGS((st_table *));
...
st_table *
st_init_table3(ST_PFICPCP compare, ST_PFICPI hash)
{
...
int
st_delete_int(st_table *table, char **keyp, int *value)
{
    int hash_val;
    char *key = *keyp;
    st_table_entry *ptr, **last;
    ...
    *last = ptr->next;
    if (value != NIL(int)) *value = (long) ptr->record;
    *keyp = ptr->key;
    FREE(ptr);
    table->num_entries--;
    return 1;
...
int 
st_gen_int(st_generator *gen, char **key_p, int *value_p)
{
    int i;
